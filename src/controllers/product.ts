...
import { getFileNameFromBase64 } from "../utils/cloudinary";



export const fetchProducts = async (req: Request, res: Response) => {
  try {
    const { category, brand, priceRange, size, quality, search, limit, page = 1 } = req.query;
    const query: any = {};
...
export const searchProducts = async (req: Request, res: Response) => {
  try {
    const { category, minPrice, maxPrice, size, quality, brand, productName } = req.query;
    const query: any = {};
...
export const addProduct = async (req: Request, res: Response) => {
  try {
    ...
    if (!productName || !actualPrice || !size || !images || !category || !quality) {
    ...
    const imageUrlsAfterUploading = await Promise.all(images.map(async (image: string) => {
